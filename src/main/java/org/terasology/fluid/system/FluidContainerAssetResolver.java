...
/**
 * Generates TextureData for the asset system by combining fluid textures with fluid container item textures, or just by
 * tiling fluid textures to a specified size.
 */
...
public class FluidContainerAssetResolver implements AssetDataProducer<TextureData> {
     ...
     * @return A set of the names of modules for which this system can produce asset data with the given resource name
     */
    @Override
    ...
    public Optional<TextureData> getAssetData(ResourceUrn urn) throws IOException {
        ...
        if (parameters.length != (isItem ? 6 : 1)) {
            logger.warn("Unexpected number of tokens when trying to getAssetData for a fluid container's content: {}"
                    , parameters);
            return Optional.empty();
        }
        ...
        parameters[parameters.length - 1] = parameters[parameters.length - 1].substring(0,
                parameters[parameters.length - 1].length() - 1);
            ...
            try {
                ...
                for (int x = min.x; x < min.x + size.x; x += fluidWidth) {
                    for (int y = min.y; y < min.y + size.y; y += fluidHeight) {
                        ...
                        graphics.drawImage(fluidTexture, x, y, x + fluidTileWidth, y + fluidTileHeight, 0, 0,
                                fluidTileWidth, fluidTileHeight, null);
                    }
                }
        ...
        TextureData data = new TextureData(result.getWidth(), result.getHeight(), new ByteBuffer[]{resultBuffer},
